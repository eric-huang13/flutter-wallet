...
class _CollectionScreenState extends State<CollectionScreen> {
                    ...
                    return GestureDetector(
                      onTap: () {
                        if (assets[index].type == NftType.TYPE_RECIPE) {
                          ...
                          Navigator.of(context).push(MaterialPageRoute(builder: (_) => AssetDetailViewScreen(nftItem: assets[index]))).then((_) => {walletsStore.setStateUpdatedFlag(true)});
                        }
                      },
                      child: ClipRRect(
      ...
      if (_colType == collectionType[0].title) {
        ...
        cookbooks.forEach((element) async {
          ...
          recipes.forEach((recipe) {
            ...
            if (nft.appType.toLowerCase() == "avatar" && cookbooks.any((cookbook) => cookbook.iD == nft.cookbookID)) {
              setState(() {
                assets.add(nft);
              });
            }
  ...
  Future<void> onRecipeClicked(NFT asset) async {
    ...
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => PurchaseItemScreen(nft: asset))).then((_) => {walletsStore.setStateUpdatedFlag(true)});
  }
}
